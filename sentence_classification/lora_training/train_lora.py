...
from transformers import AutoModelForSequenceClassification, AutoTokenizer, DataCollatorWithPadding, AutoConfig, get_linear_schedule_with_warmup
...
config = load_config("config_lora.yaml", "train")


class SentClassificationEncoder:
    def __init__(self, model_checkpoint, label2id, encoder_max_len: int = 512):
        self.tokenizer = AutoTokenizer.from_pretrained(model_checkpoint, do_lower_case=True)      # load the tokenizer
...
def fit(model, train_loader: DataLoader, dev_loader: DataLoader, opt: torch.optim.Optimizer, scheduler, adapter_save_path: str, 
        adapter_info_path: str, monitor: str = "accuracy"):
    ...
    saved_adapter_folder : str
        Folder location to save model.
    adapter_info_path : str
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if dev_metric_results[monitor] > monitor_val_max: 
                ...
                model.save_pretrained(f"{adapter_save_path}")
                write_best_model_info(epoch+1, dev_metric_results[monitor], model_info_path=adapter_info_path)
            else: 
                pass
...
if __name__ == "__main__":
    ...
    encoder = SentClassificationEncoder(config.base_model_checkpoint, label2id)
    ...
    # set model config
    base_model_config = AutoConfig.from_pretrained(config.base_model_checkpoint,
    ...
    base_model = AutoModelForSequenceClassification.from_pretrained(config.base_model_checkpoint, config=base_model_config)

    # save base model and tokenizer in one folder
    base_model.save_pretrained(config.base_model_save_path)
    encoder.tokenizer.save_pretrained(config.base_model_save_path)

    # Set LoRA config
    logger.info("Set LoRA config")
    ...
    fit(model, train_dl, dev_dl, optimizer, scheduler, adapter_save_path=config.adapter_save_path, 
        adapter_info_path= config.adapter_info_path, monitor='f1')
